...
def tile_area(area):
	...
	if unit_is_meter:
		...
		hag_cmd = ['wine', os.path.join(lastools_path, 'lasheight.exe'), '-i', os.path.join(meter_dir, area + '/*.laz'), '-olaz', '-odir', os.path.join(hag_dir, area), '-cores', str(mp.cpu_count()), '-skip_files']	# Use meter_dir for calculating HAG
	else:
		hag_cmd = ['wine', os.path.join(lastools_path, 'lasheight.exe'), '-i', os.path.join(raw_dir, area + '/*.la*'), '-olaz', '-odir', os.path.join(hag_dir, area), '-cores', str(mp.cpu_count()), '-skip_files']
		
	# Following steps happen during the tiling:
	# 1. generate HeightAboveGround feature
	# 2. merge all files
	# 3. rescale intensity
	# 4. tile with buffer
	...
	print(f'{area}: Tiling ...')
	tile_cmd = ['wine', os.path.join(lastools_path, 'lastile64.exe'), '-i', os.path.join(hag_dir, area + '/*.laz'),  '-merged', '-scale_intensity', intensity_scale_factor, '-tile_size', tile_length, '-buffer', tile_buffer, '-olaz', '-odir', os.path.join(tiled_dir, area)]
	subprocess.run(tile_cmd)
	print('Tiled area: ', os.path.join(tiled_dir, area))
	
def create_feature(area):
	print(f'{area}: Creating features ...')
	# Create dataset folders
...
def split_dataset(area):
	...
	header=["X","Y","Z","ReturnNumber","NumberOfReturns","UserData", "Intensity", "NormalX", "NormalY", "NormalZ", "Classification"]	# UserData: HeightAboveGround
